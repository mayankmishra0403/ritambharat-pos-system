...
export const createPosOrder = async (req, res, next) => {
    try {
        ...
        sendWhatsAppToStaff(restaurantId, `🆕 ${order.table?.name || '#'+order.orderNumber} - New order`, ['OWNER', 'WAITER']);
...
export const processPayment = async (req, res, next) => {
    try {
        ...
        sendWhatsAppToStaff(order.restaurant, `💰 ${order.table?.name || '#'+order.orderNumber} - Paid (${paymentMethod})`, ['OWNER', 'WAITER']);
