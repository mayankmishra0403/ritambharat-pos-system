...
export const updateOrderStatus = async (req, res, next) => {
    try {
        ...
        if (status === 'ACCEPTED') {
            ...
            sendWhatsAppToStaff(order.restaurant, `🍽️ Served – ${order.table?.name || '#'+order.orderNumber}`, ['WAITER', 'OWNER']);
...
export const updateOrderPayment = async (req, res, next) => {
    try {
        ...
        if (order.paymentStatus === 'PAID') {
            ...
            sendWhatsAppToStaff(order.restaurant, `💰 Payment Received – ${order.table?.name || '#'+order.orderNumber}`, ['OWNER', 'WAITER']);
        }
