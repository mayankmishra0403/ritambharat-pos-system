...
const ModernPOS = ({ order, restaurant, settings, type = 'display' }) => {
    ...
    return (
        <div className={`bg-white text-black ${isPrint ? 'max-w-[72mm] mx-auto font-sans text-[8pt] leading-tight' : 'p-5 font-sans text-xs leading-snug'}`}>
            ...
            {/* ─── PAYMENT ─── */}
            <div className="border-t border-dashed border-black mt-1 pt-1 font-bold text-black flex justify-end">
                <span className={`${isPrint ? 'text-[8pt]' : 'text-[10px]'} ${order.paymentStatus === 'PAID' ? 'text-green-700 font-bold' : 'text-black'}`}>{order.paymentStatus || 'PENDING'}</span>
            </div>
