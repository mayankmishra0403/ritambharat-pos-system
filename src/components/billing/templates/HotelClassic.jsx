    ...
    return (
        <div className={`bg-white text-black ${isPrint ? 'max-w-[72mm] mx-auto font-mono leading-tight' : 'p-5 font-mono text-xs leading-snug'}`}>
            ...
            <div className="text-center pb-3">
                ...
                <h1 className={`${isPrint ? 'text-[18pt]' : 'text-xl'} font-black tracking-wider`}>
                    {restaurant.name}
                </h1>
                {restaurant.tagline && (
                    <p className={`${isPrint ? 'text-[8pt]' : 'text-xs'} text-gray-600 italic mt-0.5`}>{restaurant.tagline}</p>
                )}
                <p className={`${isPrint ? 'text-[7.5pt]' : 'text-xs'} text-gray-600 mt-1`}>
                ...
                {restaurant.contact?.phone && <p className={`${isPrint ? 'text-[7.5pt]' : 'text-xs'} text-gray-600`}>{restaurant.contact.phone}</p>}
                {restaurant.alternatePhone && <p className={`${isPrint ? 'text-[7.5pt]' : 'text-xs'} text-gray-600`}>{restaurant.alternatePhone}</p>}
                {settings.showGstin && restaurant.gstin && <p className={`${isPrint ? 'text-[8pt]' : 'text-xs'} font-bold mt-0.5`}>GSTIN: {restaurant.gstin}</p>}
            </div>

            {/* ═══════════ DOUBLE LINE + TAX INVOICE ═══════════ */}
            <div className="border-t-2 border-gray-900 mb-0.5"></div>
            <div className="border-t-2 border-gray-900 mb-1"></div>
            <h2 className={`${isPrint ? 'text-[12pt]' : 'text-base'} font-bold text-center tracking-widest pb-1`}>
                {settings.invoiceTitle || 'TAX INVOICE'}
            </h2>
            <div className="border-t border-gray-500 mb-1"></div>

            {/* ═══════════ ORDER INFO ═══════════ */}
            <div className={`${isPrint ? 'text-[8pt]' : 'text-xs'} mb-1`}>
                <div className="flex justify-between">
                    <span>Invoice No</span>
                    <span className={`${isPrint ? 'text-[10pt]' : 'text-sm'} font-bold`}>{invoiceNum}</span>
                </div>
                <div className="flex justify-between">
                    <span>Date</span>
                ...
                <div className="flex justify-between">
                    <span>Type</span>
                    <span className="font-bold">{order.orderType || 'DINE IN'}</span>
                </div>
                {order.table?.name && (
                    <div className="flex justify-between">
                        <span>Table</span>
                ...
                {settings.showPax && order.pax && (
                    <div className="flex justify-between">
                        <span>PAX</span>
                        <span className="font-bold">{order.pax}</span>
                    </div>
                )}
                {settings.showWaiterName && order.waiterName && (
                    <div>Waiter: <span className="font-bold">{order.waiterName}</span></div>
                )}
                {settings.showCashierName && order.cashierName && (
                    <div>Cashier: <span className="font-bold">{order.cashierName}</span></div>
                )}
            ...
            <div className="mb-1">
                ...
                <div className={`flex font-bold py-0.5 ${isPrint ? 'text-[8.5pt]' : 'text-xs'}`}>
                ...
                {(order.items || []).map((item, idx) => (
                    <div key={idx} className={`border-b border-dashed border-gray-200 py-0.5 ${isPrint ? 'text-[8.5pt]' : 'text-xs'}`}>
                        <div className="flex">
            ...
            {/* ═══════════ GRAND TOTAL ═══════════ */}
            <div className="border-t-[3px] border-gray-900 mt-1 mb-1"></div>
            <div className={`text-center py-1 ${isPrint ? '' : ''}`}>
                <div className={`${isPrint ? 'text-[12pt]' : 'text-lg'} font-bold tracking-wider`}>GRAND TOTAL</div>
                <div className={`${isPrint ? 'text-[14pt]' : 'text-2xl'} font-black mt-0.5`}>{currency}{total.toFixed(2)}</div>
            </div>
            <div className="border-t-[3px] border-gray-900 mt-1 mb-1"></div>

            {/* ═══════════ AMOUNT IN WORDS ═══════════ */}
            {settings.showAmountInWords && total > 0 && (
                <div className={`border-t border-dashed border-gray-300 pt-1 mb-1 ${isPrint ? 'text-[8pt]' : 'text-xs'} italic text-gray-700`}>
            ...
            {/* ═══════════ PAYMENT ═══════════ */}
            <div className="border-t border-gray-500 border-b border-gray-500 py-1 mb-1">
                <div className="flex justify-between items-center">
                    <span className={`${isPrint ? 'text-[8.5pt]' : 'text-xs'} font-bold`}>{order.paymentMethod || '-'}</span>
                    {isPaid ? (
                        <span className={`${isPrint ? 'text-[9pt]' : 'text-sm'} font-bold text-green-800 bg-green-100 px-1 py-0.5 border border-green-700`}>
                            {'\u2713'} PAID
                        </span>
                    ) : (
                        <span className={`${isPrint ? 'text-[8.5pt]' : 'text-xs'} text-gray-500`}>{order.paymentStatus || 'PENDING'}</span>
                    )}
                </div>
            </div>
            ...
            {/* ═══════════ FOOTER ═══════════ */}
            {settings.showFooter && (
                <div className={`text-center pt-2 ${isPrint ? 'text-[7.5pt]' : 'text-xs'} text-gray-600`}>
                    <div className="border-t-2 border-gray-900 mb-1"></div>
